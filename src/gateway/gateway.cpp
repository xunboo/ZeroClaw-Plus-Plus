...
std::string hash_webhook_secret(const std::string &value) {
  ...
  SHA256(reinterpret_cast<const unsigned char *>(value.c_str()), value.size(),
         hash);
...
std::string whatsapp_memory_key(const std::string &sender,
                                const std::string &id) {
...
std::string nextcloud_talk_memory_key(const std::string &sender,
                                      const std::string &id) {
...
SlidingWindowRateLimiter::SlidingWindowRateLimiter(uint32_t limit_per_window,
                                                   ...
                                                   size_t max_keys)
    : limit_per_window_(limit_per_window), window_(window),
      max_keys_(std::max(max_keys, size_t(1))),
      last_sweep_(std::chrono::steady_clock::now()) {}

void SlidingWindowRateLimiter::prune_stale(
...
bool SlidingWindowRateLimiter::allow(const std::string &key) {
  ...
  if (now - last_sweep_ >=
      std::chrono::seconds(RATE_LIMITER_SWEEP_INTERVAL_SECS)) {
    prune_stale(cutoff);
    last_sweep_ = now;
  }
...
GatewayRateLimiter::GatewayRateLimiter(uint32_t pair_per_minute,
                                       ...
                                       size_t max_keys)
    : pair_(pair_per_minute, std::chrono::seconds(RATE_LIMIT_WINDOW_SECS),
            max_keys),
      webhook_(webhook_per_minute, std::chrono::seconds(RATE_LIMIT_WINDOW_SECS),
               max_keys) {}
...
IdempotencyStore::IdempotencyStore(std::chrono::seconds ttl, size_t max_keys)
    : ttl_(ttl), max_keys_(std::max(max_keys, size_t(1))) {}
...
bool verify_whatsapp_signature(const std::string &app_secret,
  ...
  HMAC(EVP_sha256(), app_secret.c_str(), app_secret.size(), body.data(),
       body.size(), result, &len);
...
std::optional<std::string> run_gateway(const std::string &host, uint16_t port,
                                       zeroclaw::config::Config config) {
  // Initialize filesystem assets (load from disk at runtime)
  static_files::initialize_filesystem_assets("./web/dist");

  httplib::Server svr;

  // Set up request handlers
  svr.Get("/health", [](const httplib::Request &req, httplib::Response &res) {
    // Simple health check for now - in full implementation would use
    // handle_health
  ...
  svr.Get(R"(^/_app/(.*)$)",
          [](const httplib::Request &req, httplib::Response &res) {
            http::Request low_req;
            low_req.path = req.path;
            auto low_res = static_files::handle_static(low_req);
            res.status = static_cast<int>(low_res.status_code);
            res.set_content(low_res.body, low_res.content_type.c_str());
            for (const auto &[k, v] : low_res.headers.headers) {
              res.set_header(k.c_str(), v.c_str());
            }
          });

  // SPA Fallback (handle 404 by serving index.html for GET requests)
  svr.set_error_handler(
      [](const httplib::Request &req, httplib::Response &res) {
        if (res.status == 404 && req.method == "GET") {
          http::Request low_req;
          low_req.path = req.path;
          auto low_res = static_files::handle_spa_fallback(low_req);
          res.status = static_cast<int>(low_res.status_code);
          res.set_content(low_res.body, low_res.content_type.c_str());
          for (const auto &[k, v] : low_res.headers.headers) {
            res.set_header(k.c_str(), v.c_str());
  ...
  std::cout << "ZeroClaw Gateway listening on http://" << host << ":" << port
            << "\n";
  std::cout << "Web Dashboard: http://" << host << ":" << port << "/\n";

  if (!svr.listen(host.c_str(), port)) {
    return "Failed to start server on " + host + ":" + std::to_string(port);
  }
